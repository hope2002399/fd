...
$.extend(Datepicker.prototype, {
	...
	_updateInput: function(target, keyUp) {
		...
		if (inst) {
			...
			if (onSelect && !keyUp && !inst.inSelect) {
				...
				onSelect.apply(target, [value,prevalue,inst.selectedDates]);
				inst.inSelect = false;
			}
		}
	},
	...
	_checkOffset: function(inst) {
		...
		/**解决 thickbox中日期选择控件错位问题*/
		//if($.browser.msie){
		//	var parentObj = $(window.parent.document).find("#TB_iframeContent");
		//	if(parentObj && parentObj.length>0){
		//		offset.top = offset.top + scrollY;
		//	}
		//}
