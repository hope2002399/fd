...
<div class="content">
    <table class="table table-bordered table-striped table-hover">
        ...
        <tr>
            <th><span>id</span><span class="label label-important">*</span></th>
            <th><span>class（用于输入框样式控制、填写校验，多个以空格分隔）</span></th>
        </tr>
        <tr>
            <td>
                <input type="text" id="tagid"  placeholder="必填项"  />
            </td>
            <td>
        ...
        <tr>
            <th colspan="2"><span>style</span></th>
        </tr>
        <tr>
            <td colspan="2">
        ...
        <tr>
            <th><span>onselected(日期选择)</span></th>
            <th><span>onchange(输入框)</span></th>
        </tr>
        <tr>
            <td>
                 <input type="text" id="tagonselected" >
            </td>
            <td>
               <input  type="text"  id="tagonchange"/>
            </td>
        </tr>
        <!-- <tr>
            <th colspan="2">
...
<script type="text/javascript">
    ...
    window.onload = function () {
        if (UE.plugins[thePlugins].editdom) {  // 编辑
            ...
            $G('taglabel').value = oNode.getAttribute('taglabel');
			   		...
			   		$G('tagonselected').value  = oNode.getAttribute('tagonselected');  
			   		$G('tagonchange').value  = oNode.getAttribute('tagonchange');
	...
	if ($G('taglabel').value == '') {
		...
		var tagonselected = $G('tagonselected').value;
		var tagonchange = $G('tagonchange').value;
	/* 	var tagOtherAttrs = $G('tagOtherAttrs').value; */
		...
		if (!oNode) { // 新增
			...
			oNode = createElement('input', 'fdField');
			oNode.setAttribute('name', 'fdField');
			oNode.setAttribute('type', 'text');
			// tagType
			oNode.setAttribute('tagType', 'datepick');
		} else {
			opFlag = "edit";
		}
		...
		try {
			...
			oNode.setAttribute('tagonselected', tagonselected);
			oNode.setAttribute('tagonchange', tagonchange);
