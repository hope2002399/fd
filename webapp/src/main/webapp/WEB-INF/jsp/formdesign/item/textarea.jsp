...
<div class="content">
    <table class="table table-bordered table-striped table-hover">
        ...
        <tr>
            ...
            <td>
               <input id="tagmaxlength" type="number" />
            </td>
        </tr>
        <tr>
            <th colspan="2">
...
<script type="text/javascript">
    ...
    window.onload = function () {
        if (UE.plugins[thePlugins].editdom) {  // 编辑
            ...
            $G('taglabel').value = oNode.getAttribute('taglabel');
			   		...
			   		$G('tagmaxlength').value  = oNode.getAttribute('tagmaxlength');  
			   		$G('tagOtherAttrs').value  = oNode.getAttribute('tagOtherAttrs'); 
         }else{
        	   // 默认
            $G('tagclass').value =  "w470 h82 " ;
              }
   	 	}
	...
	if ($G('taglabel').value == '') {
		...
		var tagmaxlength = $G('tagmaxlength').value;
		...
		if (!oNode) { // 新增
			...
			oNode = createElement('textarea', 'fdField');
			// tagType
			oNode.setAttribute('tagType', 'textarea');
			oNode.innerHTML = '多行文本框' ;
			oNode.setAttribute("rows" , "6") ;
			oNode.setAttribute("style" , "width:95%;") ;
		} else {
			opFlag = "edit";
		}
		...
		try {
			...
			oNode.setAttribute('tagmaxlength', tagmaxlength);
